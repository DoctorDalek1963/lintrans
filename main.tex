...
% Number figures and equations based on sections
\usepackage{etoolbox}
...
\newcommand\replace[3]{\directlua{
	tex.sprint(string.gsub(\luastring{#1}, \luastring{#2}, \luastring{#3}))
}}
...
\newcommand{\pytestScreenshot}[1]{%
\begin{figure}[H]
	\centering
	\includegraphics[width=0.98\linewidth]{development/#1/pytest.png}
	\caption{Running \texttt{pytest} with the new tests}
	\label{fig:development:#1:pytest.png}
\end{figure}
}
