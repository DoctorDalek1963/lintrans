...
The \texttt{lintrans.matrices.utility} module will look like this:
...
class MatrixWrapper:
    def __init__(self):
        # This dictionary maps all letters of the alphabet to an optional matrix
    ...
    def __getitem__(self, name: str) -> Optional[MatrixType]:
        ...
        Using ``__getitem__`` here allows for syntax like ``wrapper['A']`` as if it was a dictionary.
        """
    ...
    def __setitem__(self, name: str, new_matrix: Optional[MatrixType]) -> None:
        ...
        If ``new_matrix`` is None, then that effectively unsets the matrix name.

        Using ``__getitem__`` here allows for syntax like ``wrapper['A'] = matrix`` as if it was a dictionary.
        """
...
In unit testing, I will test the validation, parsing, and generation of rotation matrices from an angle. I will also unit test the utility functions for the GUI, like \texttt{is\_valid\_float}, which is needed to verify input when defining a matrix visually.
