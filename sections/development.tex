...
I could make this class subclass \mintinline{python}{dict}, since it's basically just a dictionary at this point, but I want to extend it with much more functionality later, so I chose to handle the dictionary stuff myself.
...
These test lots of simple expressions, but don't test any more complicated expressions, nor do they test any validation, mostly because validation doesn't really exist at this point. \mintinline{python}{'A++B'} is still a valid expression and is equivalent to \mintinline{python}{'A+B'}.
...
Here, we test some valid data, some definitely invalid data, and some edge cases. At this stage, \mintinline{python}{'A--1B'} was considered a valid expression. This was a quirk of the validator at the time, but I fixed it later. This should obviously be an invalid expression, especially since \mintinline{python}{'A--B'} is considered invalid, but \mintinline{python}{'A--1B'} is valid.

The \mintinline{python}{@pytest.mark.parametrize} decorator on line 32 means that \texttt{pytest} will run one test for valid inputs, and then another test for invalid inputs, and these will count as different tests. This makes it easier to see which tests failed and then debug the app.
...
When implementing the parser, I first had to tighten up validation to remove anomalies like \mintinline{python}{'A--1B'} being valid. I did this by factoring out the optional minus signs from being part of a number, to being optionally in front of a number. This eliminated this kind of repetition and made \mintinline{python}{'A--1B'} invalid, as it should be.
...
\fsbsr{development/2ade98ac28d1c3f6691e4afa819142a3ab8e9fd9/gui.png}{Parallel lines being drawn for matrix $1.2\mathbf{I}$}{This worked quite well when the matrix involved no rotation, as seen in Figure~\ref{fig:development:2ade98ac28d1c3f6691e4afa819142a3ab8e9fd9:gui.png}, but this didn't work with rotation. When trying \mintinline{python}{'rot(45)'} for example, it looked the same as in Figure~\ref{fig:development:37e7c208a33d7cbbc8e0bb6c94cd889e2918c605:gui.png}.\par Also, the vectors aren't particularly clear. They'd be much better with arrowheads on their tips, but this is just a prototype. The arrowheads will come later.\par My next step was to make the transformed grid lines work with rotations.}
...
Unfortunately, the system I use to render matrices is still quite bad at its job. This makes it hard to test properly. But, transformations like \mintinline{python}{'2rot(90)'} work exactly as expected, which is very good.
...
I then created the \texttt{is\_matrix\_too\_big()} method to just check that the elements of the matrix are within the desired bounds. If it returns \mintinline{python}{True} when we try to render or animate, then we call \texttt{show\_error\_message()}.
