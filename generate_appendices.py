...
def generate_files(directory: str, type_: str) -> None:
    ...
    tree = git.Repo('lintrans').commit('63f569328a1906976d725fa72da02e98e5d73afb').tree

    with open(f'sections/appendices/{type_}_code.tex', 'w', encoding='utf-8') as f:
        f.write(HEADER)

        for item in tree.traverse():
            if type(item) == git.objects.Blob and item.path.startswith(directory) and item.path.endswith('.py'):
