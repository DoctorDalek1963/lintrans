...
- A single matrix is written as a capital letter like ``A``, or as ``rot(x)``, where ``x`` is a real number
...
.. note::
   ``A^2 3B`` will be interpreted as ``A^{23}B``, not ``A^{2}3B``. Braces are needed to clarify this.
   However, ``A^2 + 3B`` will be interpreted as expected.

Here is the technical BNF schema used by :func:`lintrans.matrices.parse.parse_matrix_expression` and
:func:`lintrans.matrices.parse.validate_matrix_expression`:
